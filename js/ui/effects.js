...
export function spawnParticles(host, {
  kind = 'bubble',
  count = 8,
  palette = [200, 205, 210],
  drift = 24,
...
function clearSinkTimers(boardEl) {
  const existing = sinkSequenceTimers.get(boardEl);
  if (!existing) return;
...
export function sinkShipCells(boardEl, cells) {
  ...
  const cellNodes = cells
    .map(([row, col]) => getCell(boardEl, row, col))
    .filter(Boolean);
  ...
  if (!reducedMotion) {
    const pulseTimer = setTimeout(() => {
      restartClass(boardEl, 'board--sink-pulse', 250);
    }, boardPulseDelay);
...
function removeParticleLayer(layer) {
...
export function showVictoryConfetti() {
  ...
  setTimeout(() => removeParticleLayer(layer), lifetime + 400);
}
